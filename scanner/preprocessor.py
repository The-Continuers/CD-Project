...
class Preprocessor:
    ...
    def replace_micros(self, data, macros):
        for key, value in macros.items():
            ...
            data = re.sub(
                fr'([^A-Za-z0-9_])?{self.tof_key}([^A-Za-z0-9_])?', f'\g<1>{value}\g<2>', data)
    ...
    def _preprocess(self) -> str:
        ...
        for line in data.splitlines():
            ...
            if arr[0] == "define":
                ...
                is_import_line = bool(
                    import_file_match) or bool(import_line_match)
                if not bool(import_file_match):
                    if not is_import_line:
                        selected_line = True
                    new_data_parts.append(line)
                else:
                    new_data_parts.append(
                        self.handle_import_file(import_file_match))
